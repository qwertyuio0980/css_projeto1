...
public class ReserveIndTicHandler {
	...
	public List<String> getSeatsOfEventDate(String chosenDay) throws ApplicationException {
        ...
        List<IndividualTicket> indTickets = tc.getTicketByDayAndEvent(this.currentEvent, correspondingED);
	...
	public List<String> reserveSeats(List<String> chosenSeats, String emailAddress) throws ApplicationException {
		...
		for(Ticket t : this.currentEvent.getTickets()) {
		...
		for (Seat seat : chosenSeatsList) {
			for(IndividualTicket t : tc.getTicketsByEventAndDateAndSeat(this.currentEvent, this.currentEventDate, seat)) {
				t.setSold();
				ticketList.add(t);
			}
		}
